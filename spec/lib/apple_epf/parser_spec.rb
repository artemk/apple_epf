...
describe AppleEpf::Parser do
  ...
  describe "perform" do
    ...
    describe "#process_rows" do
      ...
      it 'should be able to start from desired position and do n number of records' do
        pos, eof = parser.process_rows(limit:2) {nil}
        expect(pos).to be_kind_of(Integer)
        expect(eof).to be false
        expect { |b|
         parser.process_rows(pos: pos, &b)
        }.to yield_successive_args(*[third_entry])
        pos, eof = parser.process_rows(pos: pos) {nil}
